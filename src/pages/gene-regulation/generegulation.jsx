    ...
    () => [
      {
        ...
        columns: [
          ...
          },
          {
            ...
            //filterSelectOptions: ["UPR", "DPR", "UNR", "DNR"],
          },
        ],
      },

      {
        ...
        columns: [
          {
            accessorKey: "Gene Symbol From", //normal accessorKey
  ...
  useEffect(() => {
    ...
    if (
      ...
      nodes = geneRegulationGraph.nodes.map((node) => ({
        ...
        kd: node.kd,
        lineStyle: {
          opacity: node.kd ? Math.min(0.4118 * node.kd + 1.0235, 1) : 0.5,
        },
      ...
      edges = geneRegulationGraph.edges.map((edge) => ({
        ...
        value: edge.value,
        lineStyle: {
          opacity: Math.min(0.4118 * Math.abs(edge.value) + 0.1765, 1),
        ...
        tooltip: {
          formatter: edge.id.includes("+cor+")
            ? "Corr. R: " +
              edge.value +
              "<br />" +
              edge.id.replace("+cor+", " ~ ") +
      ...
      for (let m = edges.length - 1; m > -1; m--) {
        if (
          edges[m].type2 === "Corr" &&
          !geneRegulationCoreSettings.include_corr
        )
          continue;
        else if (
          edges[m].type2 === "Exp" &&
          !geneRegulationCoreSettings.include_exp
        )
          continue;
        if (
          edges[m].source.startsWith("non-targ") ||
          edges[m].target.startsWith("non-targ")
        )
          continue;
        if (
          geneRegulationCoreSettings.filter1Enabled &&
          edges[m].source !== geneRegulationCoreSettings.selectedGene
        ) {
          if (
            (edges[m].type === "UPR" ||
              edges[m].type === "UPR_DPR" ||
              edges[m].type === "UNR_DNR" ||
              edges[m].type === "UNR_UNR" ||
          ...
          if (
            (edges[m].type === "UNR" ||
              edges[m].type === "UNR_DPR" ||
              edges[m].type === "UPR_DNR" ||
              edges[m].type === "UPR_UNR" ||
        ...
        if (
          ...
          edges[m].target !== geneRegulationCoreSettings.selectedGene
        ) {
          if (
            (edges[m].type === "UPR_DPR" ||
              edges[m].type === "UNR_DNR" ||
        ...
        if (
          edges[m].type === "UNR_DNR" &&
          (!geneRegulationCoreSettings.unr_dnr ||
            !geneRegulationCoreSettings.unr ||
            !geneRegulationCoreSettings.dnr)
        )
          continue;
        if (
          edges[m].type === "UNR_DPR" &&
          (!geneRegulationCoreSettings.unr_dpr ||
            !geneRegulationCoreSettings.unr ||
            !geneRegulationCoreSettings.dpr)
        )
          continue;
        if (
          edges[m].type === "UPR_DNR" &&
          (!geneRegulationCoreSettings.upr_dnr ||
            !geneRegulationCoreSettings.upr ||
            !geneRegulationCoreSettings.dnr)
        )
          continue;
        if (
          edges[m].type === "UPR_DPR" &&
          (!geneRegulationCoreSettings.upr_dpr ||
            !geneRegulationCoreSettings.upr ||
            !geneRegulationCoreSettings.dpr)
        )
          continue;
        if (
          edges[m].type === "UPR_UNR" &&
          (!geneRegulationCoreSettings.upr_unr ||
            !geneRegulationCoreSettings.unr ||
            !geneRegulationCoreSettings.upr)
        )
          continue;
        if (
          edges[m].type === "UNR_UPR" &&
          (!geneRegulationCoreSettings.unr_upr ||
            !geneRegulationCoreSettings.unr ||
            !geneRegulationCoreSettings.upr)
        )
          continue;
        if (
          edges[m].type === "DPR_DNR" &&
          (!geneRegulationCoreSettings.dpr_dnr ||
            !geneRegulationCoreSettings.dpr ||
            !geneRegulationCoreSettings.dnr)
        )
          continue;
        if (
          edges[m].type === "DNR_DPR" &&
          (!geneRegulationCoreSettings.dnr_dpr ||
            !geneRegulationCoreSettings.dnr ||
            !geneRegulationCoreSettings.dpr)
        )
          continue;

        if (edges[m].type === "UPR" && !geneRegulationCoreSettings.upr)
          continue;
        if (edges[m].type === "DPR" && !geneRegulationCoreSettings.dpr)
          continue;
        if (edges[m].type === "UNR" && !geneRegulationCoreSettings.unr)
          continue;
        if (edges[m].type === "DNR" && !geneRegulationCoreSettings.dnr)
          continue;
        if (
          edges[m].type === "DPR_DPR" &&
          (!geneRegulationCoreSettings.among_dpr ||
            !geneRegulationCoreSettings.dpr)
        )
          continue;
        if (
          edges[m].type === "UPR_UPR" &&
          (!geneRegulationCoreSettings.among_upr ||
            !geneRegulationCoreSettings.upr)
        )
          continue;
        if (
          edges[m].type === "DNR_DNR" &&
          (!geneRegulationCoreSettings.among_dnr ||
            !geneRegulationCoreSettings.dnr)
        )
          continue;
        if (
          edges[m].type === "UNR_UNR" &&
          (!geneRegulationCoreSettings.among_unr ||
            !geneRegulationCoreSettings.unr)
        )
          continue;
        if (
          edges[m].type2 === "Exp" &&
          Math.abs(edges[m].value) < geneRegulationCoreSettings.absoluteZScore
        )
          continue;
        if (
          edges[m].type2 === "Corr" &&
          Math.abs(edges[m].value) < geneRegulationCoreSettings.corr_cutoff
        )
          continue;
        ...
        toBeRemoved.forEach((node) => {
          ...
          if (neighbors) {
            neighbors.forEach((neighborId) => {
              ...
              const temp = nodesFiltered.find(
                (element) => element["id"] == neighborId
              );
              if (temp) {
                temp["neighbourCount"]--;
                if (
                  temp.id !== geneRegulationCoreSettings.selectedGene &&
                  temp.neighbourCount <
                    geneRegulationCoreSettings.neighbourCount
                ) {
      ...
      if (geneRegulationCoreSettings.onlyLinked === true) {
        ...
        const currentNodeIds = new Set(
          nodesFiltered.map((object) => object.id)
        );
        edgesFiltered.forEach((edge) => {
          //const [source, target] = edge;
          let source = edge["source"];
      ...
      if (geneRegulationCoreSettings.basedOnFinal === true) {
        ...
        if (geneRegulationCoreSettings?.filter5Enabled === true) {
          ...
          edgesFiltered = edgesFiltered.filter(
            (edge) =>
              (edge.source === geneRegulationCoreSettings.selectedGene ||
                edge.target === geneRegulationCoreSettings.selectedGene) &&
              nodeCounts[edge.target] +
                nodeCounts[edge.source] -
                nodeCounts[geneRegulationCoreSettings.selectedGene] >=
                geneRegulationCoreSettings.filterCount5
          );
        ...
        nodesFiltered.forEach((node) => {
          ...
          if (id === geneRegulationCoreSettings.selectedGene) {
            ...
            node["symbolSize"] =
              minSize +
              ((nodeCounts[id] - minValue) / (maxValue - minValue)) * sizeRange;
          }
        });
      }
      ...
      for (let m = edgesFiltered.length - 1; m > -1; m--) {
        if (
          !idSet.has(edgesFiltered[m].source) ||
          !idSet.has(edgesFiltered[m].target)
        )
          continue;

        tableInfo.push({
          "Regulation Type": edgesFiltered[m].type2,
          Direction: edgesFiltered[m].type,
          "Gene Symbol From": edgesFiltered[m].source,
          "Gene Symbol To": edgesFiltered[m].target,
      ...
      if (geneRegulationCoreSettings.layout === "none") {
        ...
        g.setGraph({
          ...
          //width:100,
          ranksep:
            geneRegulationCoreSettings.dagreSeperation === 0
              ? 1
              : geneRegulationCoreSettings.dagreSeperation,
          nodesep: 100,
        });
      ...
      setOptions({
        ...
        legend: [
          {
            data: [
              "Upstream Positive Regulator",
              "Upstream Negative Regulator",
              "Downstream Positively Regulated",
              "Downstream Negatively Regulated",
            ],
            top: "top",
            itemGap: 5,
            left: "left",
        ...
        series: [
          {
            ...
            links: edgesFinal,
            categories: [
              { name: "Upstream Positive Regulator" },
              { name: "Upstream Negative Regulator" },
              { name: "Downstream Positively Regulated" },
              { name: "Downstream Negatively Regulated" },
            ],
