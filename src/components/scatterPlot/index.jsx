...
const ScatterPlot = ({ pcaGraph , graphmapSettings, scatterplotSettings}) => {
  ...
  if(pcaGraph["clusterCount"]>0)
  {
    ...
    {
      data.push([pcaGraph["PC1"][i],pcaGraph["PC2"][i],pcaGraph["PC3"][i],pcaGraph["GeneSymbols"][i],pcaGraph["clusterLabels"][i],pcaGraph["clusterProb"][i]]);
    }
  }
  else
  {
    for (var i = 0; i < pcaGraph["GeneSymbols"].length; i++) 
    {
      data.push([pcaGraph["PC1"][i],pcaGraph["PC2"][i],pcaGraph["PC3"][i],pcaGraph["GeneSymbols"][i],-1,1]);
    }
  }
