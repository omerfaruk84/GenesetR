...
const headings  = ['Regulation Type', 'From','To','Score','Source NC', 'Target NC', 'Total NC']
const helps = {'Regulation Type':'Cor: Correlation; Exp: Expression; Int: Protein-Protein Interaction',               
...
const PathFinder = ({ pathFinderGraph , pathfinderSettings}) => {
  ...
  const edges = pathFinderGraph.edges.map(edge => ({
      ...
      value: edge.value,
      lineStyle: {
        opacity:Math.min(((1-pathfinderSettings.minEdgeopacity)/0.8)*Math.abs(edge.value) + ((pathfinderSettings.minEdgeopacity-0.2)/0.8),1), //y = 0.813x + 0.187 //ax+b = y // ((1-pathfinderSettings.minEdgeopacity)/0.8) + ((pathfinderSettings.minEdgeopacity-0.2)/0.8)
        width:Math.max(2.222*Math.abs(edge.value) +3.556 ,4), 
        color:edge.type === "Cor"?'rgb(25, 25, 250)':(edge.value < -0.2  ? 'rgb(25, 206, 17)': (edge.value > 0.2  ? 'rgb(255, 1, 1)':'rgb(123, 123, 123)')),
        type: edge.type === "Cor"? 'dotted': (edge.type === "Int"  ? 'dashed':'solid'),
        curveness: edge.type === "Cor"? 0.3: (edge.type === "Int"  ? 0.5:0.4),
      },
      
      tooltip: {
        formatter: edge.type === "Cor"? 'Corr. R: '+  edge.value +'<br />' + edge.id.replace("+cor+"," ~ ") : (edge.type === "Int"  ? 'Protein-Protein Intereation: ':'Effect: '+  edge.value +'<br />' + edge.id.replace("+"," → ")),
      },      
      //value: edge.value,
  }));
  ...
  for(let edge in edges){     
    ...
    } else if(edges[edge].type === "Int" && pathfinderSettings.BioGridData === false){
      continue;      
    }else 
    {     
...
edgesFiltered.forEach(edge => {
  ...
  maxNodeCount = Math.max(nodeCounts[source], nodeCounts[target], maxNodeCount)
});
...
nodesFiltered.forEach(node => {
  const { id } = node;
  if (nodeCounts[id]) {
    node["symbolSize"] = ((pathfinderSettings.maxNodeSize-8)/(maxNodeCount-1))*nodeCounts[id]+ (8*maxNodeCount - pathfinderSettings.maxNodeSize)/(maxNodeCount-1);
    node["neighbours"] = nodeCounts[id];
    node["label"] = {show: nodeCounts[id]>pathfinderSettings?.showLabels,  position: nodeCounts[id]>6? "inside":'right' , color: nodeCounts[id]>6? "black":'black'};
    ...
    node["itemStyle"] = {
      ...
      opacity:node["kd"]? Math.min(((1-pathfinderSettings.minNodeopacity)/0.8)*Math.abs(node["kd"]) + ((pathfinderSettings.minNodeopacity-0.2)/0.8),1):pathfinderSettings.minNodeopacity, //y = 0.813x + 0.187 //ax+b = y // ((1-pathfinderSettings.minEdgeopacity)/0.8) + ((pathfinderSettings.minEdgeopacity-0.2)/0.8)
...
if(pathfinderSettings.layout ==="none"){ 
  ...
  g.setGraph({      
    ...
    //width:500,
    nodesep:100,
    ranksep:(pathfinderSettings.dagreSeperation)===0?1:(pathfinderSettings.dagreSeperation)
  });
...
{ 
  if(!idSet.has(edgesFinal[m].source) || !idSet.has(edgesFinal[m].target)) 
    continue;

tableInfo.push({'Regulation Type':edgesFinal[m].type, 'From': edgesFiltered[m].source,'To':edgesFiltered[m].target,'Score': edgesFiltered[m].value, 'Source NC': nodeCounts[edgesFiltered[m].source], 'Target NC': nodeCounts[edgesFiltered[m].target], 'Total NC': nodeCounts[edgesFiltered[m].target] +nodeCounts[edgesFiltered[m].source ]})
}
