...
function hashGeneList(geneList) {
    ...
    return fetch(`https://maayanlab.cloud/Enrichr/enrich?userListId=${listID}&backgroundType=${datasets}`, {
        method: 'GET'
    })
    .then(response => {
        if (response.ok) {
    ...
    .catch(error => {
        toast({
            message: { "type":  "Error", "icon": true, "heading": "Enrichr", "content": "Sorry. Error fetching enrichment results. Enrichr servers are not responding." },
            autoClose: 2000
        });
        throw error;
    });
